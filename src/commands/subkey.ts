...
export function registerSubkeyCommand(program: Command, ctx: AppContext): void {
    ...
    .action(async (label: string) => {
      ...
      try {
        ...
        outputResult({
          ...
          usdc: usdcBalance !== null ? `${formatUnits(usdcBalance, USDC_DECIMALS)} USDC.e` : null,
  ...
  sk.command('fund')
    ...
    .option('--usdc <amount>', 'USDC.e amount, e.g. "3.0"')
      ...
      async (
        ...
        try {
          ...
          if (account.chainId !== subkey.boundChainId) {
            throw new Error(
              `Subkey bound to chain ${subkey.boundChainId}, but active account is on chain ${account.chainId}. Switch with \`elytro account switch\`.`,
            );
          }
          ...
          if (opts.usdc) {
            if (subkey.boundChainId !== 137) {
              throw new Error(
                'USDC.e transfer is currently only wired for Polygon (137). Use --native for other chains.',
              );
            }
            const amountWei = parseUnits(opts.usdc, USDC_DECIMALS);
            ...
            txs.push({ to: USDC_POLYGON, value: '0x0', data });
          }
  ...
  sk.command('sweep')
    ...
    .action(async (label: string, opts: { usdcOnly?: boolean; nativeOnly?: boolean }) => {
      ...
      try {
        ...
        const pendingNonceHex = await walletClient.request({
          method: 'eth_getTransactionCount',
          params: [subkey.id, 'pending'],
        });
        let nonce = parseInt(pendingNonceHex as string, 16);

        const result: {
          usdc: { amount: string; txHash: Hex } | null;
          native: { amount: string; txHash: Hex } | null;
        } = { usdc: null, native: null };

        // ── USDC sweep first (costs gas on subkey) ──
        if (!opts.nativeOnly && subkey.boundChainId === 137) {
          const usdcBal = (await ctx.walletClient.readContract({
            address: USDC_POLYGON,
            abi: ERC20_ABI,
            functionName: 'balanceOf',
            args: [subkey.id],
          ...
          if (usdcBal > 0n) {
            spinner.text = `Sweeping ${formatUnits(usdcBal, USDC_DECIMALS)} USDC.e → smart account...`;
            ...
            result.usdc = { amount: formatUnits(usdcBal, USDC_DECIMALS), txHash };
          }
        }
        ...
        if (!opts.usdcOnly) {
          ...
          try {
            const gasHex = await walletClient.request({
            ...
            estimatedGas = BigInt(gasHex as string);
          } catch (err) {
            throw new Error(
